...
# I got this solution when refactoring my previous very confusing solution. I don't feel like it's dynamic programming, but it is pretty clean. Basically, I had super_digit(9 / 10) = 0 here. I did try to make a variable sum to save to sum of the current digits, but didn't get it to work.
# super_digit(9875)
...
# Space Complexity - O(n) call stack memory
# Feel kinda off to use a while loop in a recursion, but if statement does not work no matter what I tried. Stop at 12 for k = 3 and n = 148. Not sure why it's not recursive like the previous one. I feel like converting the number to a string will be easier, but I didn't want the extra space.
